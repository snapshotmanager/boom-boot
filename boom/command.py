...
def _cache_image(img_path: str, backup: bool, update: bool = False) -> str:
    ...
    try:
        ...
        _log_error("Could not cache path %s: %s", img_path, err)
        raise
    return ce.path
...
def _create_cmd(
    ...
    try:
        ...
        return 1
    except (ValueError, BoomConfigError, OSError) as err:
    ...
    try:
        ...
        __write_legacy()
    except (BoomError, OSError, ValueError) as err:  # pragma: no cover
        if cmd_args.debug:
            raise
        print(err, file=sys.stderr)
        return 1
...
def _write_legacy_cmd(
    ...
    except (BoomLegacyFormatError, BoomError, OSError, ValueError) as err:
        print(err, file=sys.stderr)
        return 1
    return 0
...
def main(args: List[str]) -> int:
    ...
    if cmd_args.root_lv:
        try:
            ...
            print(err, file=sys.stderr)
            print(f"Invalid logical volume name: '{cmd_args.root_lv}'", file=sys.stderr)
            return 1
    ...
    if cmd_args.debug:
        ...
        try:
            status = command[1](cmd_args, select, opts, identifier)
        except (BoomError, OSError, ValueError) as err:
            _log_error("Command failed: %s", err)

    if bc.cache_enable and bc.cache_auto_clean:
        try:
            clean_cache()
        except (BoomError, OSError) as err:
